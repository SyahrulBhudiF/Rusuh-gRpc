...
impl DbPort<UserInfo> for UserInfoAdapter {
    async fn save(&self, data: UserInfo) -> Result<(), Error> {
        sqlx::query(
            "INSERT INTO user_info (id, user_id, first_name, last_name, gender, birth_date, created_at, updated_at) VALUES ($1, $2, $3, $4, $5, $6, $7)",
        )
        .bind(&data.id)
        .bind(&data.user_id)
    ...
    async fn find_by_coll(&self, coll: &str, value: &str) -> Result<Option<User>, Error> {
        let query = match coll {
            ...
            "first_name" => {
                "SELECT * FROM user_info WHERE first_name = $1"
            }
            "last_name" => {
                "SELECT * FROM user_info WHERE last_name = $1"
            }
            "gender" => {
